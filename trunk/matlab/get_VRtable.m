...
%Default call: VRtable = getVRtable(paraEx, paraSt)
%------
%Input:
%------
...
for m = numBS:-1:2	
	for n = sum(num_BSCC(end:-1:m+1))+1:sum(num_BSCC(end:-1:m))
		...
		%Check if BS's VR density is reached
		for nBS = 1:numBS
...
for m = 1:length(VRtable(1,:,1))
	if VRtable(1,m,2)==0 %If the VR doesn't have a cluster
        ...
        if length(find(VRtable(1,:,2)==0))==0
            continue; %After attributing the last VR
        end
        
		num_VRgroup = poissrnd(paraSt.MS_common); %Number of extra VRs in the VR group
		if num_VRgroup ==0
            ...
            while num_VRgroup > numVREmp || num_VRgroup ==0 %If there are not enough VRs to group
                num_VRgroup = poissrnd(paraSt.MS_common);
            end
