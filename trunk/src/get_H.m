...
%Pulse shaping of H
num=rcosine(1,paraEx.overSample,'fir/normal'); %Raised cosine filter para.
HOld=H;
clear H
switch type
    case 'narr'
        for m=1:M
            for n=1:N
               h=HOld(m,n);
               H(:,m,n)=rcosflt(h,1,paraEx.overSample,'filter/Fs',num);
            end
        end
    case 'wide'
        for m=1:M
