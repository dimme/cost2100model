...
if length(active_VR)>0 %If there are active far cluster
    ...
    for m = 1:length(active_c)
        ...
        for n = 1:paraSt.n_mpc
            ...
            amp_h = a_VR(m)*sqrt( cluster(active_c(m)).shadow_f*a_c )*mpc(active_c(m)).a_mpc(n)*pathloss*exp(-j*2*pi*paraEx.freq*tau_mpc);
...
if BS.cluster_local.active
    for n = 1:paraSt.n_mpc
        ...
        amp_h = sqrt( BS.cluster_local.shadow_f )*BS.mpc_local.a_mpc(n)*pathloss*exp(-j*2*pi*paraEx.freq*tau_mpc);
...
if MS.cluster_local.active
    for n = 1:paraSt.n_mpc
        ...
        amp_h = sqrt( MS.cluster_local.shadow_f )*MS.mpc_local.a_mpc(n)*pathloss*exp(-j*2*pi*mod(paraEx.freq*tau_mpc,1));

        h( num_actc*paraSt.n_mpc+1+n,: ) = [phi_mpc_BS theta_mpc_BS phi_mpc_MS theta_mpc_MS tau_mpc amp_h]; %The unfiletered unsampled impulse response, angle in radii
    end
end
